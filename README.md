...
- a check out action.
- a setup node action.
and then followed by an upload action to attach the artifacts to the release/tag.
...
All the inputs are also set as outputs. Plus the installer filename generated by `electron-builder` is set. This filename is used to set both the artifact and asset variables for use by the release/tag attach process (`svenstaro/upload-release-action@v1-release`).
...
# Appendix A - Example Electron `package.json`
...
This is taken from Github repo `mandolyte/rrouter/.github/workflows/main.yml`
